# 08_capital_interior.py
from bigquery_client import *
import pandas as pd
import os

# ── Leitura do REGIC e geração dinâmica do CASE WHEN ──────────────────────────
regic = pd.read_csv("data/raw/regic_2018_hierarquia.csv")

# Gera linhas do CASE WHEN para cada cidade do REGIC
regic_cases = "\n    ".join([
    f"WHEN t.cidade = '{row.cidade}' AND t.estado = '{row.estado}' THEN '{row.categoria}'"
    for _, row in regic.iterrows()
])
...
SELECT
  -- Colunas originais (A→AW) — ordem preservada
  t.dataValor,
  t.pais,
  t.filial,
  ...
  -- Colunas novas (AX em diante) — Excel antigo não quebra
  t.estado,
  t.cidade,
  t.populacao_cidade,
  CASE 
  ...
  -- ← NOVO: classificação Capital x Interior via REGIC 2018
  CASE
    {regic_cases}
    ELSE 'Interior'
...
df.to_csv("data/raw/08_eop_capital_interior.csv", index=False)

print("✅ Relatório gerado com sucesso!")
print(f"📊 Cidades classificadas como Capital/Metrópole: {(df['capital_interior'] != 'Interior').sum()}")
print(df[['cidade', 'capital_interior']].drop_duplicates().sort_values('capital_interior').to_string())
